  ...
  mutations: {
    ...
    async compile(state) {
      if (state.code == state.buffer) {
        return (state.output = state.temp);
      } else if (state.code == "" || state.code == " ") {
        return (state.output =
          "no hi ha cap codi a compilar");
      } else {
    ...
    async compileOut(state) {
      if (state.codeOut == state.buffer) {
        return (state.outputOut = state.temp);
      } else if (state.code == "" || state.codeOut == " ") {
        return (state.outputOut =
          "no hi ha cap codi a compilar");
      } else {
        state.outputOut = "compilando...";
        state.buffer = state.codeOut;
        const res = await axios.post(`${state.API}addon/compile`, state.codeOut, {
          headers: {
            "Content-Type": "text/plain",
            title: `temp_file_${state.seed}_${(state.time.hour,
            state.time.min,
    ...
    async downloadOut(state) {
      if (state.codeOut === "" || state.codeOut === " ") {
        return (state.codeOut = "> codigo vacio! (empty)");
      }
      await axios
        .post(`${state.API}addon/download`, state.codeOut, {
          headers: {
            "Content-Type": "text/plain",
            title: `temp_file_${state.seed}_${(state.time.hour,
            state.time.min,
